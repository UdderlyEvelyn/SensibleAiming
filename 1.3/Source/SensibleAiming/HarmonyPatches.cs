...
namespace SA
{
	[HarmonyPatch(typeof(ShotReport), "AimOnTargetChance_IgnoringPosture")]
	//[HarmonyPatch(typeof(Verb_LaunchProjectile), "TryCastShot")]
    //[HarmonyPatch(typeof(ShotReport), "HitFactorFromShooter", new Type[] { typeof(Thing), typeof(float) })]
    class HarmonyPatches
    {
		...
		//static bool Prefix(ref float __result, Thing caster, float distance)

		//{
		//	float multiplier = 1.1f;
		//	__result = ShotReport.HitFactorFromShooter((caster is Pawn) ? caster.GetStatValue(StatDefOf.ShootingAccuracyPawn) * multiplier : caster.GetStatValue(StatDefOf.ShootingAccuracyTurret) * multiplier, distance);
		//	return false;
		//}
